...
%>
  ...
  <tr>
    ...
    <td><%=i_mes %></td>
    <td><%=i_add %></td>
    <td><a href="<%=request.getContextPath() %>/item/item_change.jsp?i_name=<%=i_name %>&i_image=<%=i_image %>&i_mes=<%=i_mes %>&i_add=<%=i_add %>">수정하기</a></td>
    <td><a href="<%=request.getContextPath() %>/item/delete_item.jsp">삭제하기</a></td>
  </tr>
  </table>
		 </div>
		 <!-- End Right Column -->
